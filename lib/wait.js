...
export function patchWaitAgentSchema(tool) {
  ...
  if (tool.parameters?.properties?.timeout_ms) {
    tool.parameters.properties.timeout_ms.description =
      'Wait duration in milliseconds. Defaults to 300000 (5 minutes). Do not pass short polling intervals (e.g. 15s/30s); wait_agent is event-driven and returns immediately when a teammate completes a task or sends a message.'
  }
}
