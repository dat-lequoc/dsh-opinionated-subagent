 ...
 *
 * Registers one card into the shipped `settings.plugins.tab` slot keyed by this
 * plugin's settings namespace. The Built-in plugins settings page renders the
 * intersection of the namespaces the Host serves and the cards registered into that slot,
 * so this
 ...
 *
 * Reads and writes go through `ctx.configForms.get`, which fences each write with
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      ctx.slots.inject('tool.call.toolview', () => ctx.slots.register(
        { name: 'tool.call.toolview', key: TOOL_NAME, priority: -1 },
        SubagentModelRow,
      ))
    }
